...
void RestfulPostsCtrlBase::getOne(
...
void RestfulPostsCtrlBase::updateOne(
    const HttpRequestPtr &req,
    std::function<void(const HttpResponsePtr &)> &&callback,
    Posts::PrimaryKeyType &&id) {
  ...
  try {
    if (isMasquerading()) {
      if (!Posts::validateMasqueradedJsonForUpdate(*jsonPtr,
                                                   masqueradingVector(), err)) {
        Json::Value ret;
        ret["error"] = err;
        auto resp = HttpResponse::newHttpJsonResponse(ret);
        resp->setStatusCode(k400BadRequest);
...
void RestfulPostsCtrlBase::deleteOne(
    const HttpRequestPtr &req,
    std::function<void(const HttpResponsePtr &)> &&callback,
    Posts::PrimaryKeyType &&id) {
  auto dbClientPtr = getDbClient();
...
void RestfulPostsCtrlBase::create(
    const HttpRequestPtr &req,
    std::function<void(const HttpResponsePtr &)> &&callback) {
  auto jsonPtr = req->jsonObject();
  if (!jsonPtr) {
  ...
  if (isMasquerading()) {
    if (!Posts::validateMasqueradedJsonForCreation(*jsonPtr,
                                                   masqueradingVector(), err)) {
      Json::Value ret;
      ret["error"] = err;
      auto resp = HttpResponse::newHttpJsonResponse(ret);
...
/*
void RestfulPostsCtrlBase::update(const HttpRequestPtr &req,
...
RestfulPostsCtrlBase::RestfulPostsCtrlBase()
    : RestfulController({"id", "title", "created_at"}) {
  /**
